...
### 9.1 프리미엄 실패/성공 확률 보정(피티)
- 목적: AI 호출 실패나 비용 제한으로 인한 “실패” 발생 시 사용자의 좌절을 줄이고 재도전 유도.
- 기본 로직(초안):
  - 최초 성공 확률: 70%
  - 실패 1회당 +10%p 누적, 최대 100%까지 상승.
  - 성공 시 보정값을 0으로 리셋.
- 상태 키: `deviceId` 기준(로그인 시 `userId` 전환). 하루 단위로 리셋 가능.
- API 응답 예시 필드 추가 고려: `{ success: boolean, currentSuccessRate, attemptsToday }`
- 실패 처리:
  - 실패 시에도 fallback 캐릭터를 내려주고, “다음 성공 확률이 올라갔어요” 정도의 소프트 안내.
  - 확률/수치를 과하게 노출하지 않고 게이지/아이콘으로 표시.

### 9.2 카테고리별 파츠 해제(광고 기반 확장)
- 기본 노출: 카테고리당 30개까지만 선택 목록에 노출.
- “더보기” 흐름: 더보기 버튼 선택 시 광고 시청 → 시청 완료 후 해당 카테고리에 +10개씩 추가 노출. 여러 번 시청 가능(카테고리별 독립), 상한은 전체 파츠 수까지.
- 상태 관리: `deviceId` 기준(로그인 시 `userId`)으로 카테고리별 unlockedCount 저장. 기본 30, 광고 1회당 +10.
- API 제안:
  - `GET /parts/unlocked?deviceId=...` → `{ categories: { animal: 30, insect: 40, ... } }`
  - `POST /ad/claim-parts?category=animal` → 광고 완료 후 해당 카테고리 unlockedCount += 10 (상한은 카테고리 총 파츠 수)
- 클라이언트 UX:
  - 파츠 리스트에 “더보기(광고로 추가)” CTA 표시. 광고 진행 중 스피너, 실패 시 조용히 토스트.
  - 해제된 개수/총 개수 표기(예: 30/60).
  - 멀티 디바이스 시 서버 값을 우선으로 표시, 로컬 캐시 동기화.

### 9.3 프리미엄 다시 뽑기(광고 후 재생성)
- 목적: AI 결과가 마음에 들지 않을 때 재시도를 허용하되, 비용을 광고로 보전.
- 흐름:
  1) 프리미엄 뽑기 결과 화면에서 “다시 뽑기(광고)” 버튼 노출.
  2) 광고 시청 완료 → 동일 파츠 조합으로 AI 재생성 요청(`/generate-character` 재호출, premium=true).
  3) 재생성 성공 후 결과 교체, 피티/일일 프리미엄 사용량은 재시도에도 동일하게 카운트(정책 결정 필요).
- 정책 옵션:
  - A) 재시도도 프리미엄 사용량 1회로 카운트(가장 단순).
  - B) 최초 뽑기에서만 카운트, 재시도는 추가 카운트 없이 광고만 요구.
  - 문서 기본값: A(일관성 우선). B로 전환 시 API에 `reroll=true` 전달하여 사용량 미증가 처리 필요.
- 서버 요구사항:
  - 광고 검증: `/ad/claim-reroll` 또는 adToken으로 재시도 허용 여부 확인.
  - 피티 처리: 재시도가 실패하면 피티 추가 여부를 명확히(기본: 실패 시 피티 +1, 성공 시 리셋).
  - 응답에 `rerollAttempt: true/false` 포함 고려.
- 클라이언트 UX:
  - 광고 완료 전까지 기존 결과 유지, 광고 종료 후 로딩 스피너 → 새 결과 교체.
  - 재시도 실패 시 이전 결과를 유지하고 소프트 안내(“이번엔 다시 쉬어가기”).
...
## 17. 상단 고정형 광고(배너)
- 목적: AI 호출 비용 보조를 위한 상시 노출 배너.
- 위치: 주요 화면(Home, 파츠 선택, 결과, 도감) 상단 영역에 고정 배치.
- 사이즈/레이아웃:
  - 고정 높이(예: 60~90px) 확보 후 배너 로딩, 플레이스홀더색으로 자리 유지 → 레이아웃 시프트 방지.
  - Safe area/top inset 고려(iOS notch 등).
- 요청 주기/갱신:
  - 화면 진입 시 1회 로드, 앱 포그라운드 복귀 시 갱신 가능(과도한 리프레시 방지).
  - 실패 시 빈 상태/재시도 버튼 없이 조용히 실패 처리.
- UX/정책:
  - 프리미엄 유저(유료 혹은 프리미엄 세션)에는 배너 숨김 옵션 고려.
  - 클릭 영역은 배너 내부로만 제한(다른 버튼 오작동 방지).
  - 광고 로딩 중에도 주요 버튼은 즉시 사용 가능.
- 기술:
  - 초기엔 더미 이미지/테스트 배너로 자리 확보, 추후 SDK(AdMob/ironSource 등) 연동.
  - 네트워크 실패 시 로그만 남기고 UI는 placeholder 유지.

## 18. 비용 최적화 전략
- 모델 선택
  - OpenAI 고비용 모델 대신 Stability SD3.5-lite, Flux.1 등 경량 모델 우선 검토 (이미지 1장 당 10~20원 수준 목표).
  - 해상도 256~320px 생성 후 경량 업스케일러로 512px 확장.
- 프리셋/캐싱
  - 인기 조합은 사전 생성 후 CDN에서 서빙, 실시간 호출 비중 축소.
  - 동일 파츠 조합 요청 시 캐시 히트하도록 parts_key 기반 저장.
- 배치/큐
  - 무료 뽑기는 큐에 넣어 배치 inference, 프리미엄만 실시간.
  - 야간 배치로 신규 프리셋 생성해 낮 시간 호출 수 감소.
- 하이브리드 아트
  - 베이스 캐릭터/배경을 정적 에셋으로 두고 얼굴/컬러만 AI로 생성.
  - 파츠별 레이어 PNG 조합 방식 병행해 완전 AI 생성 비중 축소.
- 레이아웃 가이드
  - 기준 폭 390dp, 최소 지원 320dp
  - 콘텐츠 최대 폭 480dp. 그 이상에서는 중앙 정렬 + 좌우 여백 확보
  - 가로/태블릿에서도 세로 레이아웃 유지, 콘텐츠 폭 480dp 고정
- **브랜드/세계관: 미쑤리 (MiSSuri)**
  - 컨셉: 섞이고 섞여 태어나는 몽글몽글 친구들. Mix → MiSSuri.
  - UX 톤: 어린이가 친숙하게 부를 수 있는 이름, 캐릭터 IP 확장성(애니메이션, 굿즈 등) 강조.
  - 세계관: “미쑤리 나라”의 여러 파츠가 사는 마을에서 합성을 통해 새로운 친구가 탄생. 도감을 채우면 “미쑤리의 숲” 등이 열린다.
